...
unless(defined(&_STRUCT_TIMESPEC)) {
    ...
    if((defined(&__WORDSIZE) ? &__WORDSIZE : undef) == 64|| (defined (&__SYSCALL_WORDSIZE)  && (defined(&__SYSCALL_WORDSIZE) ? &__SYSCALL_WORDSIZE : undef) == 64) || ((defined(&__TIMESIZE) ? &__TIMESIZE : undef) == 32 && !defined (&__USE_TIME64_REDIRECTS))) {
    } else {
