...
unless(defined(&_BITS_FLOATN_H)) {
    ...
    require 'features.ph';
    if((defined (&__x86_64__) ?  &__GNUC_PREREQ (4, 3) : (defined (&__GNU__) ?  &__GNUC_PREREQ (4, 5) :  &__GNUC_PREREQ (4, 4))) || ( &__glibc_clang_prereq (3, 9)  && (!defined (&__INTEL_LLVM_COMPILER) || !defined (&SYCL_LANGUAGE_VERSION)))) {
	...
	if((defined(&__HAVE_FLOAT128) ? &__HAVE_FLOAT128 : undef)) {
	    if(! &__GNUC_PREREQ (7, 0) || (defined (&__cplusplus)  && ! &__GNUC_PREREQ (13, 0)) ||  &__glibc_clang_prereq (3, 9)) {
	    }
